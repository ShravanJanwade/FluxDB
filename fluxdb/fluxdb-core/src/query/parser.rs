...
use super::{
    AggregateFunc, Assignment, CompareOp, Condition, DeleteStatement, FromClause, GroupBy,
    JoinClause, JoinCondition, JoinType, OrderBy, OrderByItem, Query, QueryValue, SelectItem,
    SetOpType, SetOperation, Statement, UpdateStatement, WhereClause,
};
use crate::{FluxError, Result, TimeRange};
use sqlparser::ast::{
    BinaryOperator, Expr, Function, FunctionArg, FunctionArgExpr, Join, JoinConstraint,
    JoinOperator, Query as SqlQuery, Select, SelectItem as SqlSelectItem, SetExpr, SetOperator,
...
impl QueryParser {
    ...
    pub fn parse_statement(sql: &str) -> Result<Statement> {
        ...
        let statements =
            Parser::parse_sql(&dialect, sql).map_err(|e| FluxError::SqlParse(e.to_string()))?;
        ...
        match &statements[0] {
            SqlStatement::Query(query) => Self::parse_query_to_statement(query),
            SqlStatement::Update {
                table,
                assignments,
                selection,
    ...
    fn parse_set_expr(expr: &SetExpr) -> Result<Statement> {
        match expr {
            ...
            SetExpr::Query(query) => Self::parse_query_to_statement(query),
            SetExpr::SetOperation {
                op,
                set_quantifier,
                left,
    ...
    fn parse_query(query: &SqlQuery) -> Result<Query> {
        ...
        {
            return Err(FluxError::SqlParse(
                "WITH, FETCH, LIMIT BY, locking and output clauses are not supported".into(),
            ));
        }
    ...
    fn parse_select_to_query(
        ...
        {
            return Err(FluxError::SqlParse(
                "Unsupported SELECT modifier; see the documented SQL subset".into(),
            ));
        }
    ...
    fn parse_from(select: &Select) -> Result<FromClause> {
        if select.from.len() > 1 {
            return Err(FluxError::SqlParse(
                "Only one FROM source is supported".into(),
            ));
        }
    ...
    fn parse_table_factor(tf: &TableFactor) -> Result<FromClause> {
        match tf {
            TableFactor::Table { name, alias, .. } => {
                if name.0.len() != 1 {
                    return Err(FluxError::SqlParse("Use a single measurement name".into()));
                }
    ...
    fn parse_join(left: FromClause, join: &Join) -> Result<FromClause> {
        ...
        let (join_type, on) = match &join.join_operator {
            ...
            JoinOperator::FullOuter(constraint) => (
                JoinType::FullOuter,
                Self::parse_join_constraint(constraint)?,
            ),
            JoinOperator::CrossJoin => (JoinType::Cross, JoinCondition::Natural),
                ...
                SqlSelectItem::ExprWithAlias { expr, alias } => {
                    ...
                    } = item
                    {
                        *a = Some(alias.value.clone());
                        ...
                        return Err(FluxError::SqlParse(
                            "Aliases are supported for aggregates only".into(),
                        ));
                    }
                    items.push(item);
                }
    ...
    fn parse_function(func: &Function) -> Result<SelectItem> {
        ...
        {
            return Err(FluxError::SqlParse(
                "Aggregate FILTER, OVER, ordering and null-treatment clauses are not supported"
                    .into(),
            ));
        }
        ...
        if func.args.len() != 1 || func.distinct || agg_func == AggregateFunc::Percentile {
            return Err(FluxError::SqlParse(
                "Use a single field argument; DISTINCT aggregates and PERCENTILE are not supported"
                    .into(),
            ));
        }
        let field = if func.args.is_empty() {
            ...
            match &func.args[0] {
                ...
                _ => return Err(FluxError::SqlParse("Unsupported function argument".into())),
            }
        };

        if field == "*" && agg_func != AggregateFunc::Count {
            return Err(FluxError::SqlParse("Only COUNT accepts *".into()));
        }
        ...
        match expr {
            ...
            Expr::UnaryOp { op, expr } => {
                ...
                match op {
                    ...
                    _ => Err(FluxError::SqlParse(
                        "Unsupported unary operator in value".into(),
                    )),
                }
            }
            _ => Err(FluxError::SqlParse(format!(
        ...
        match val {
            Value::Number(n, _) => {
                if n.contains('.') || n.contains('e') || n.contains('E') {
                    Ok(QueryValue::Float(n.parse().map_err(|_| {
                        FluxError::SqlParse("Invalid float".into())
                    })?))
                } else {
                    Ok(QueryValue::Integer(n.parse().map_err(|_| {
                        FluxError::SqlParse("Invalid integer".into())
                    })?))
                }
            }
    ...
    fn parse_comparison(left: &Expr, op: &BinaryOperator, right: &Expr) -> Result<Condition> {
        ...
        if field.to_lowercase() == "time" {
            ...
            let range = match compare_op {
                CompareOp::Gt => TimeRange::new(
                    ts.checked_add(1)
                        .ok_or_else(|| FluxError::SqlParse("Timestamp overflow".into()))?,
                    i64::MAX,
                ),
                CompareOp::Ge => TimeRange::new(ts, i64::MAX),
                CompareOp::Lt => TimeRange::new(
                    i64::MIN,
                    ts.checked_sub(1)
                        .ok_or_else(|| FluxError::SqlParse("Timestamp overflow".into()))?,
                ),
                CompareOp::Le => TimeRange::new(i64::MIN, ts),
                CompareOp::Eq => TimeRange::new(ts, ts),
                CompareOp::Ne => {
                    return Ok(Condition::Not(Box::new(Condition::TimeRange(
                        TimeRange::new(ts, ts),
                    ))))
                }
        ...
        if let Expr::Value(Value::SingleQuotedString(s)) = right {
            return Ok(Condition::StringCompare {
                field,
                op: compare_op,
                value: s.clone(),
            });
        ...
        if matches!(right, Expr::CompoundIdentifier(_)) {
            return Ok(Condition::TagEquals {
                tag: field,
                value: Self::extract_field_name(right)?,
            });
        }
        Ok(Condition::ValueCompare {
            field,
            op: compare_op,
            value: Self::parse_value_expr(right)?,
        })
    }
        ...
        match expr {
            ...
            Expr::Value(Value::SingleQuotedString(s)) => {
                ...
                chrono::DateTime::parse_from_rfc3339(s)
                    ...
                    .timestamp_nanos_opt()
                    .ok_or_else(|| FluxError::SqlParse("Timestamp out of range".into()))
            }
            Expr::UnaryOp {
                ...
                if let Expr::Value(Value::Number(n, _)) = expr.as_ref() {
                    ...
                    Err(FluxError::SqlParse("Invalid timestamp".into()))
                }
            }
            _ => Err(FluxError::SqlParse(
                "Unsupported timestamp expression".into(),
            )),
        }
    }
            ...
            match expr {
                Expr::Function(func) if func.name.to_string().to_lowercase() == "time" => {
                    ...
                    )))) = func.args.first()
                    {
                        ...
                        return Err(FluxError::SqlParse(
                            "Use GROUP BY time(\"1m\") with a single-quoted interval".into(),
                        ));
                    }
                }
    ...
    fn parse_interval(s: &str) -> Result<i64> {
        ...
        if num <= 0 {
            return Err(FluxError::SqlParse("Time interval must be positive".into()));
        }
        num.checked_mul(multiplier)
            .ok_or_else(|| FluxError::SqlParse("Interval overflow".into()))
    }
    ...
    fn parse_order_by(query: &SqlQuery) -> Result<Option<OrderBy>> {
        ...
        for order_expr in &query.order_by {
            if order_expr.nulls_first.is_some() {
                return Err(FluxError::SqlParse(
                    "Explicit NULLS FIRST/LAST is not supported".into(),
                ));
            }
...
mod tests {
    ...
    fn test_parse_in() {
        let query =
            QueryParser::parse("SELECT * FROM temperature WHERE sensor_id IN ('s1', 's2', 's3')")
                .unwrap();
    ...
    fn test_parse_between() {
        let query =
            QueryParser::parse("SELECT * FROM temperature WHERE value BETWEEN 20 AND 30").unwrap();
    ...
    #[test]
    fn test_parse_like() {
        let query = QueryParser::parse("SELECT * FROM temperature WHERE sensor_id LIKE 'sensor-%'")
            .unwrap();
    ...
    fn test_parse_is_null() {
        let query = QueryParser::parse("SELECT * FROM temperature WHERE value IS NULL").unwrap();
