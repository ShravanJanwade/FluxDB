...
impl StorageEngine {
    ...
    pub fn new(config: StorageConfig) -> Result<Self> {
        ...
        directory_lock.try_lock().map_err(|e| {
            FluxError::Config(format!("Data directory already locked or unavailable: {e}"))
        })?;
        let engine = Self {
    ...
    pub fn get_or_create_database(&self, name: &str) -> Result<Arc<Database>> {
        ...
        match self.create_database(name) {
            Ok(db) => Ok(db),
            Err(e) => self.get_database(name).ok_or(e),
        }
    }
    ...
    pub fn validate_name(name: &str) -> Result<()> {
                ...
                .all(|c| c.is_ascii_alphanumeric() || c == b'_' || c == b'-')
        {
            return Err(FluxError::Config(
                "Database names must use 1–64 letters, digits, underscores or hyphens".into(),
            ));
        }
    ...
    pub fn drop_database(&self, name: &str) -> Result<()> {
        ...
        if databases
            .get(name)
            .map(|db| Arc::strong_count(db) > 1)
            .unwrap_or(false)
        {
            return Err(FluxError::Config("Database is busy; retry deletion".into()));
        }

        if !databases.contains_key(name) {
            return Err(FluxError::DatabaseNotFound(name.to_string()));
        }
        // Release mapped files for Windows before atomically hiding the directory.
        // On rename failure, reopen the unchanged database while holding the registry lock.
        ...
        let removed_path = self
            .config
            .data_dir
            .join(format!(".deleted-{name}-{suffix}"));
        databases.remove(name);
        if let Err(error) = std::fs::rename(&db_path, &removed_path) {
            ...
            databases.insert(name.to_string(), Arc::new(reopened));
            return Err(error.into());
        }
        #[cfg(unix)]
        std::fs::File::open(&self.config.data_dir)?.sync_all()?;
        if let Err(error) = std::fs::remove_dir_all(&removed_path) {
            tracing::warn!(%error, path = %removed_path.display(), "Dropped database files require offline cleanup");
        }
